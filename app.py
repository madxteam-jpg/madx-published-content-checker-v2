...
st.title("🛡️ Ultimate Editorial & SEO QA Auditor (Playwright Edition)")
st.write("Enter a published or staging blog URL below to run an instant enterprise compliance audit using a real headless browser.")
...
async def fetch_page_and_check_links(target_url):
    ...
    async with async_playwright() as p:
        ...
        return {
            ...
            "html": content,
            "img_statuses": img_statuses,
            "link_data": link_data,
            "link_statuses": link_statuses
        }

if run_button and url_input:
    with st.spinner("Launching headless browser and inspecting page content..."):
        try:
            # Run async function safely within Streamlit's event loop
            audit_data = asyncio.run(fetch_page_and_check_links(url_input))
            
            if audit_data["status_code"] != 200 and audit_data["status_code"] != 0:
                st.error(f"❌ CRITICAL: Could not fetch page. HTTP Status Code: {audit_data['status_code']}")
                st.stop()
            ...
            # --- COLUMN 1: TECH STACK, META & LAYOUT ---
            with col1:
            ...
            with col2:
                ...
                broken_imgs = sum(1 for status in audit_data["img_statuses"] if status >= 400)
                ...
                for link_info, status in zip(audit_data["link_data"], audit_data["link_statuses"]):
                    if link_info["is_ext"] and "nofollow" not in link_info["rel"]:
                        ext_no_nofollow += 1
                    if status == 404:
                        if link_info["is_ext"]: broken_ext += 1
                        else: broken_int += 1
